...
function tabUpdateVisibility()
{
	var showEls = {
		...
		'catselect': (currentTab == 'list'  || currentTab == 'fileurl'  /* || currentTab == 'browser'*/),
		'listtplselect': (currentTab == 'list'),
		'catselect-filter': (currentTab == 'list'),
		'catzipurl-desc': (currentTab == 'fileurl'),		'filesort' : (currentTab == 'list'  || currentTab == 'browser'),
		'catsort':  ( (currentTab == 'list' && jQuery('#list-show-cats:checked').val())  || currentTab == 'browser'),
	};
...
function selectCat(id, a)
{
	...
	if(currentTab == 'fileurl') {
		if('function' == typeof(insCatZipTag)) {
			insCatZipTag(id, name);
		}
		return;
	}
...
function insListTag() {
	...
	var search = jQuery('#list-search').val();
	if(search)
		tag.search = search;
