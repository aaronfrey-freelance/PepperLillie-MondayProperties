...
if(!WPFB_Core::CheckPermission('upload_files|edit_file_details|delete_files'))
...
function insCatZipTag(id,name)
{
	var theTag = {"tag":"catzipurl", <?php echo WPFB_Core::$settings->use_path_tags ? '"path": getCatPath(id)' : '"id":id'; ?>};
...
function insBrowserTag()
{
 ...
 if(jQuery('#browser-private:checked').val()) tag['private'] = 1; 	if(jQuery('#browser-expanded:checked').val()) tag.expanded = 1;
	getFileSorting(tag);
	getCatSorting(tag);
...
/*function insFileformTag(nonce)
{
	var tag = {tag:currentTab};
	tag.key = nonce;
	tag.id = parseInt(jQuery('#fileform-cat').val());
	tag.overwrite = jQuery('#fileform-overwrite').is(':checked') ? '1' : '0';

	var roles = [];
	jQuery('#file_user_roles input:checked').each(function(index) {
		var r = jQuery(this).val();
		if(r != '') roles.push(r);
	});
	if(roles.length > 0)
		tag.roles = roles.join(',');

	return insertTag(tag);
}
*/

function insFileformTag(form_tag) {
	var tag = {tag:"form", id:form_tag};
	return insertTag(tag);
}
<?php }
?>
...
if(current_user_can('upload_files'))
	$tabs['fileform'] = __('Embed Upload Form', WPFB);
...
<div id="catselect" class="container">
	...
	<div id="catselect-filter">
		...
		<p id="list-search-wrap">
			<label for="list-num"><?php _e('Filter files by search keyword/tag:',WPFB) ?></label>
			<input name="list-filter" type="text" id="list-search" value="" />
		</p>	
	
	</div>
	
	<div id="catzipurl-desc">
		<p>Choose a category to create a link to a ZIP file containing all its files</p>
	</div>
...
<form id="list" class="insert">
	...
	<?php
echo " "; _e('For AJAX lists this is the number of preloaded items.',WPFB);
?>	
	</p>
...
<form id="browser" class="insert">
	...
	<p>
	<input type="checkbox" id="browser-private" name="browser-private" value="1" <?php disabled(WPFB_Core::$settings->private_files); checked(WPFB_Core::$settings->private_files); ?> />
	<label for="browser-private"><?php _e('Private Browser: Only display files owned by the logged in User.',WPFB); ?></label>
	</p>
	<p>
	<input type="checkbox" id="browser-expanded" name="browser-expanded" value="1" />
	<label for="browser-expanded"><?php _e('Expanded Browser: All Category are initially opened so all files are displayed.',WPFB); ?></label>
	</p>
...
<?php
		wpfb_loadclass('AdminGuiEmbedForms');
		set_current_screen('editor_plugin');
		$GLOBALS['hook_suffix'] = '';
	    $list_table = new WPFB_EmbeddableForms_List_Table(true, 'insFileformTag');
