...
if($frontend_upload) {	
	if($file_add_now) {
		...
		wpfb_loadclass('EmbeddedForm','File','Category');
		$embedded_form = WPFB_EmbeddedForm::Get(@$_POST['form_tag']);
		if(is_null($embedded_form) || ($msg=$embedded_form->SecurityIssues($_POST))) wpfb_ajax_die($msg);
	} else {
