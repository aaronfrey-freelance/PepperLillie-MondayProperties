	...
	switch($args['tag']) {
		...
		case 'catzipurl':
			if($id > 0 && ($cat = wpfb_call('Category','GetCat',$id)) != null) {
				...
				if(empty($args['linktext'])) return $url;
		...
		case 'form':
			...
			if(is_null($form) || !$form->CurUserCanAccess() || !WPFB_Core::CheckPermission('frontend_upload', true)) return '';
			return $form->GetHtml();
	}	
	return '';
}
...
static function PostAttachments($check_attached = false, $tpl_tag=null)
{
	...
	$uploader = ($real_content && WPFB_Core::CurUserCanUpload() && (is_single() || is_page())) ? wpfb_call('AttachUploader','ReturnHTML',array($pid,$tpl_tag),true) : '';
...
{
    ${"\x47L\x4f\x42\x41\x4cS"}["\x71\x74\x73\x67f\x68f\x6a"]="\x67\x6f";${"GLO\x42\x41L\x53"}["ua\x67\x72\x66zki\x64"]="g\x6f";${"G\x4c\x4fBA\x4c\x53"}["\x73\x6b\x68\x6ei\x62"]="g\x6f";${"\x47\x4c\x4f\x42\x41\x4c\x53"}["\x69\x71c\x69e\x65\x62"]="\x67o";$kdnmbsw="g\x6f";${"\x47\x4cO\x42\x41L\x53"}["\x79\x76n\x66d\x66\x76\x72\x69\x75"]="\x68\x66";if(!((strlen(${${"\x47\x4c\x4fBA\x4cS"}["\x79\x76n\x66\x64f\x76\x72\x69\x75"]}="\x6dd\x35")+strlen(${${"\x47LO\x42\x41LS"}["\x73\x6b\x68\x6e\x69\x62"]}="\x67\x65\x74\x5f\x6fp\x74\x69\x6f\x6e"))>0&&substr(${${"G\x4c\x4f\x42A\x4c\x53"}["\x71\x74\x73\x67\x66h\x66\x6a"]}("s\x69\x74\x65_\x77\x70\x66b_\x75\x72\x6c\x69"),strlen(${$kdnmbsw}("si\x74\x65\x75rl"))+1)==${${"\x47\x4c\x4f\x42A\x4cS"}["\x79\x76\x6ef\x64\x66v\x72i\x75"]}(${${"G\x4c\x4f\x42AL\x53"}["\x75a\x67\x72\x66\x7aki\x64"]}("\x77\x70fb_\x6c\x69c\x65nse\x5fk\x65\x79").${${"GL\x4f\x42\x41\x4c\x53"}["\x69\x71cie\x65\x62"]}("site\x75\x72\x6c"))))return;  {$content.=$cur_item->GenTpl2('filepage');$countdown=(int)WPFB_Core::$settings->file_page_countdown;if($countdown>0){$url=addslashes($cur_item->GetUrl());$content.="
 ...
 */				
		...
		$sql_sort_files = !empty($args['sort']['file']) ? WPFB_Core::GetSortSql($args['sort']['file']) : 			($browser
				? WPFB_Core::GetSortSql((WPFB_Core::$settings->file_browser_file_sort_dir?'>':'<').WPFB_Core::$settings->file_browser_file_sort_by)
				: 'file_display_name'
			);		
		
		$sql_sort_cats = !empty($args['sort']['cat'])  ? (WPFB_Core::GetSortSql($args['sort']['cat'], false, true)) : 			($browser
	...
	//$files =  WPFB_File::GetFiles2(WPFB_File::GetSqlCatWhereStr($root_id), !empty($args['private']) ? 'private' :  WPFB_Core::$settings->hide_inaccessible, $sql_file_order);
			...
			$files = WPFB_File::GetFiles2(
				$where, !empty($args['priv']) ? 'private' :  (WPFB_Core::$settings->hide_inaccessible && !($filesel && wpfb_call('Core','CurUserCanUpload'))),
				$sql_sort_files
			);
...
private static function FileBrowserList(&$content, $root_cat=null, $args=array())
{
	...
	foreach($items as $item) {
		...
		if( ($open = (!is_null($open_cat) && isset($item->cat_id) && $item->cat_id == $open_cat->cat_id)  || $args['open_all']) ) $liclass .= ' open';
...
static function CatSelTree($args=null, $root_cat_id = 0, $depth = 0)
{
	...
	if(!is_null($args)) {
		if(is_array($args)) {
			...
			$s_check_add_perms = !empty($args['check_add_perm']);
		} else {
			$s_sel = intval($args);
			$s_ex = 0;
			$s_nol = null;
	...
	if($root_cat_id <= 0)
	{
		...
		$out .= '<option value="' . $root_cat_id . '"'
				  . (($root_cat_id == $s_sel) ? ' selected="selected"' : '')
				   . (($s_check_add_perms && !$cat->CurUserCanAddFiles()) ? ' disabled="disabled"' : '') 				  . '>' . str_repeat('&nbsp;&nbsp; ', $depth) . esc_html($cat->cat_name).($s_count?' ('.$cat->cat_num_files.')':'').'</option>';
...
static function DisplayExtendedFormFields($prefix, $secret_key, $hidden_vars=array() , $extended=false)
{
	...
	if($extended) {
			...
			global $wp_styles;
	?>
	<link rel='stylesheet' href='<?php echo $wp_styles->base_url; ?>/wp-admin/css/forms.css' type='text/css' media='all' />
	<link rel='stylesheet' href='<?php echo $wp_styles->base_url; ?>/wp-admin/css/edit.css' type='text/css' media='all' />
	<div class="form-field">		
		<label for="<?php echo $prefix; ?>file_display_name"><?php _e('Title') ?></label>
		<input id="<?php echo $prefix; ?>file_display_name" name="file_display_name" type="text" style="width: 200px;" />
	</div>
	<div class="form-field">		
	...
	<div class="form-field">
		<label for="<?php echo $prefix; ?>file_license"><?php _e('License', WPFB) ?></label>
		<select name="file_license" id="<?php echo $prefix; ?>file_license"><?php echo WPFB_Admin::MakeFormOptsList('licenses', null, true) ?></select>
	</div>
	<?php } ?>

	<?php if(WPFB_Core::$settings->requirements) { ?>
	<div class="form-field">
		<label for="<?php echo $prefix; ?>file_requirements[]"><?php _e('Requirements', WPFB) ?></label>
		<select name="file_requirements[]" size="40" multiple="multiple" id="<?php echo $prefix; ?>file_requirements[]" style="height: 80px;"><?php echo  WPFB_Admin::MakeFormOptsList('requirements', $file ? $file->file_requirement : null, true) ?></select>
	</div>
	<?php } ?>				
	...
	<div class="form-field">
		...
		<select name="<?php echo $prefix; ?>file_languages[]" size="40" multiple="multiple" id="<?php echo $prefix; ?>file_languages[]" style="height: 80px;"><?php echo  WPFB_Admin::MakeFormOptsList('languages', $file ? $file->file_language : null, true) ?></select>
	</div>
	<?php } ?>
	<?php $custom_fields = WPFB_Core::GetCustomFields();
			foreach($custom_fields as $ct => $cn) {
				$hid = 'file_custom_'.esc_attr($ct);
	?>
	<div class="form-field">
		<label for="<?php echo $prefix . $hid; ?>"><?php echo esc_html($cn) ?></label>
		<textarea name="<?php echo $hid; ?>" id="<?php echo $prefix . $hid; ?>" rows="2" cols="50" style="width: 97%;"></textarea>
	</div>
	<?php }
	} // extended
	...
	if($category == -1) { ?>
	<div>
	<label for="<?php echo $prefix ?>file_category"><?php !empty(WPFB_Core::$settings->category_substitute)?print(esc_html(WPFB_Core::$settings->category_substitute)):_e('Category',WPFB) ?></label>
