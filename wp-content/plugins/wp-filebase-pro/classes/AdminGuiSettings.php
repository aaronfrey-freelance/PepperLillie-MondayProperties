...
static function Display()
{
	...
	{		
		...
		$post['file_page_url_slug'] = sanitize_title($post['file_page_url_slug']);
...
<form method="post" action="<?php echo $action_uri; ?>" name="wpfilebase-options">
	...
	if(function_exists('wp_admin_bar_render'))
		$misc_tags[] = 'admin_bar';
	array_unshift($misc_tags, 'upload_notifications');
	...
	foreach ( array_keys($wp_roles->roles) as $role ) {
		if(!in_array($role, $default_roles)) $limits[] = 'daily_limit_'.$role;
	}
	...
	$option_categories = array(
		...
		__('Display', WPFB)					=> array('file_date_format','thumbnail_size','auto_attach_files', 'attach_loop','attach_pos', 'filelist_sorting', 'filelist_sorting_dir', 'filelist_num', /* TODO: remove? 'parse_tags_rss',*/ 'decimal_size_format','search_result_tpl','disable_css','category_substitute'),
		__('File Browser',WPFB)				=> array('file_browser_post_id','file_browser_cat_sort_by','file_browser_cat_sort_dir','file_browser_file_sort_by','file_browser_file_sort_dir','file_browser_fbc', 'late_script_loading', 'folder_icon', 'small_icon_size',
			   'file_browser_expanded', 'file_browser_empty_msg', 		),
		__('Download', WPFB)				=> array('enable_cat_zip','hide_links', 'disable_permalinks', 'download_base', 'force_download', 'range_download', 'http_nocache', 'ignore_admin_dls', 'accept_empty_referers','allowed_referers' /*,'dl_destroy_session'*/,'use_fpassthru'),
		__('File Pages',WPFB)			=> array('file_page_proxy', 'file_page_countdown','file_page_url_slug','file_page_url_wfront', 'file_page_comments','file_page_gen_content'),
		...
		__('Permissions')					=> array('perm_upload_files','perm_edit_file_details','perm_delete_files','perm_create_cat','perm_delete_cat','perm_frontend_upload','perm_manage_templates','perm_manage_rsyncs','perm_manage_forms'),
		__('Templates and Scripts', WPFB)	=> array('template_file', 'template_cat', 'dlclick_js'),
		__('Sync',WPFB)						=> array('cron_sync', 'base_auto_thumb', 'remove_missing_files','fake_md5' ,'rpc_calls'),
		__('Misc')							=> $misc_tags,
		__('PDF',WPFB)						=> array('ghostscript_path','pdf_thumbnails','pdf_extract_title','pdf_title_regex')
	);
	?>
	<div id="wpfb-tabs">
