...
/**
 * Cat Zip File Download
 *
 * @param WPFB_Category $cat Category to download.
 * @return mixed Value set for the option.
 */
static function DownloadCategoryZip($cat)
{	
	$files = $cat->GetChildFiles(true, null, true);	
	
	// build a hash
	$hash_string = '';
	foreach($files as $file) { $hash_string .= $file->file_path.$file->file_size.$file->file_mtime.$file->file_hash; }
	$hash = hash('md4', $hash_string);
	unset($hash_string);	

	$dir = WPFB_Core::UploadDir().'/.tmp/';	
	$tmp_zip_file_path = $dir.$hash.".zip";
	
	if(!file_exists($tmp_zip_file_path)) {
		if(!is_dir($dir)) {
		...
		$z = new ZipArchive();
		if($z->open($tmp_zip_file_path, ZIPARCHIVE::CREATE) !== true)
			wp_die('Could not create ZIP file!');
	
		$tmp_files = array();
		$zip_dirs = array();
		$path_cut_len = strlen(trim(str_replace('\\','/',dirname($cat->GetLocalPathRel())),'/.'));	
		foreach($files as $file) {
			$entry_path = trim(substr($file->GetLocalPathRel(), $path_cut_len),'/\\');
			if($file->IsRemote()) {
				$res=wpfb_call('Admin', 'SideloadFile', $file->GetRemoteUri());
				if(!empty($res['error'])) wp_die($res['error']);
				$tmp_files[] = $local_path = $res['file'];
			} else
				$local_path = $file->GetLocalPath();
			
			if(is_file($local_path)) {
				self::zipCreateDir($z, dirname($entry_path), $zip_dirs);				
				if(!$z->addFile($local_path, $entry_path))
					wp_die('Could not add file to ZIP!');
			}
		}
		
		if(!$z->close())
			wp_die('Could not close ZIP file!');
		
		foreach($tmp_files as $tf) unlink($tf);
	}

	do_action( 'wpfilebase_catzip_downloaded', $cat->cat_id );
	
	self::SendFile($tmp_zip_file_path, array('filename' => sanitize_file_name($cat->GetTitle()).".zip"));
	
}

private static function zipCreateDir(&$z, $path, &$dirs) {
	if(isset($dirs[$path])) return $dirs[$path];
	$dir = dirname($path);
	if(!empty($dir) && $dir != '.' && !isset($dirs[$dir]))
		self::zipCreateDir($z, $dir, $dirs);
	return $dirs[$path] = $z->addEmptyDir ($path);
}
}