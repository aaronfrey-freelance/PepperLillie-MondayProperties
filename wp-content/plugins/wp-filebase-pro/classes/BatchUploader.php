...
<div id="<?php echo $this->prefix; ?>-uploader-wrap">	
	<div id="<?php echo $this->prefix; ?>-uploader-interface" class="wpfb-batch-uploader-interface">	
		...
		<form method="POST" action="" class="validate" name="batch_presets">
			<?php if(!$this->embedded_form) { ?> <h2><?php _e('Upload Presets',WPFB); ?></h2> <?php } ?>
			<?php
...
<tr class="form-field more">
	...
	<td><input name="file_display_name" id="<?php echo $prefix; ?>_display_name" type="text" value="<?php echo esc_attr($defaults['display_name']); ?>" /></td>
</tr>

<tr class="form-field more">
	<th scope="row"><label for="batch_user_roles"><?php _e('Access Permission',WPFB) ?></label></th>
	<td><!-- <input type="hidden" name="file_perm_explicit" value="1" /> --> <div id="<?php echo $prefix; ?>_user_roles"><?php WPFB_Admin::RolesCheckList('file_user_roles', $defaults['user_roles'], __('Inherit from Cat.',WPFB)) ?></div></td>
</tr>

	
<tr class="more"> <!-- class="form-field" -->
	<th scope="row"><label for="batch_direct_linking"><?php _e('Direct Linking', WPFB) ?></label></th>
	<td>		<fieldset><legend class="hidden"><?php _e('Direct Linking', WPFB) ?></legend>
				<label title="<?php _e('Yes') ?>"><input type="radio" name="file_direct_linking" value="1" <?php checked('1', $defaults['direct_linking']); ?>/> <?php _e('Allow direct linking', WPFB) ?></label>
				<label title="<?php _e('No') ?>"><input type="radio" name="file_direct_linking" value="0" <?php checked('0', $defaults['direct_linking']); ?>/> <?php _e('Redirect to post', WPFB) ?></label>

				<label title="<?php _e('No') ?>"><input type="radio" name="file_direct_linking" value="2" <?php checked('2', $defaults['direct_linking']); ?>/> <?php _e('Redirect to File Page', WPFB) ?></label>
...
<tr class="form-field more">
	<th scope="row"><label for="batch_password"><?php _e('File Password',WPFB) ?></label></th>
	<td><input name="file_password" id="<?php echo $prefix; ?>_password" type="text" value="" maxlength="16" autocomplete="off" /></td>
</tr>


<tr class="form-field more">
	<th scope="row"><label for="batch_version"><?php _e('File Version',WPFB) ?></label></th>
	<td><input name="file_version" id="<?php echo $prefix; ?>_version" type="text" value="" maxlength="32" autocomplete="off" /></td>
</tr>
...
<?php
	...
	foreach($custom_fields as $ct => $cn) {
		...
		<tr class="form-field more">
			...
			<td><textarea name="<?php echo $hid; ?>" id="<?php echo $hid; ?>" rows="1"><?php echo empty($defaults["custom_$ct"]) ? '' : esc_html($defaults["custom_$ct"]); ?></textarea></td>
		</tr>
	<?php	} ?>
...
<tr class="form-field more-more">
	...
	<td><select id="<?php echo $prefix; ?>_languages" name="file_languages[]" multiple="multiple" style="height:80px;"><?php echo  WPFB_Admin::MakeFormOptsList('languages', $defaults['languages'], true) ?></select></td>
</tr>
<?php } ?>
	
<?php if(WPFB_Core::$settings->platforms) { ?>
<tr class="form-field more-more">
	<th scope="row"><label for="batch_platforms"><?php _e('Platforms', WPFB) ?></label></th>
	<td><select id="<?php echo $prefix; ?>_platforms" name="file_platforms[]" size="40" multiple="multiple" style="height:80px;"><?php echo WPFB_Admin::MakeFormOptsList('platforms', $defaults['platforms'], true) ?></select>
	</td>
</tr>
<?php } ?>
...
<tr class="form-field more-more">
	<th scope="row"><label for="batch_requirements"><?php _e('Requirements', WPFB) ?></label></th>
	<td><select id="<?php echo $prefix; ?>_requirements" name="file_requirements[]" size="40" multiple="multiple" style="height: 80px;"><?php echo  WPFB_Admin::MakeFormOptsList('requirements', $defaults['requirements'], true) ?></select>
	</td>
</tr>
<?php } ?>
