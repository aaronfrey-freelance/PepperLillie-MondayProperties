...
class WPFB_ListTpl {
	...
	private function ParseHeaderFooter($str, $uid=null) {
		...
		if($count > 0)
		{
			$this->ajax_scroll = false; // AJAX not supported with DataTables yet!
...
, $str);
	...
	function GenerateList(&$content, $categories, $list_args=null)
	{
		...
		$search_term = empty($_GET['wpfb_s']) ? (empty($this->current_list->search)?null:$this->current_list->search) : stripslashes($_GET['wpfb_s']);
		...
		if(is_null($categories)) { // if null, just list all files!
			...
			if(count($categories) == 1 && ($cat->cat_num_files > 0 || count(WPFB_File::GetFiles2(WPFB_File::GetSqlCatWhereStr($cat->cat_id),false,null,1)))) { // single cat
	...
	{	  
		...
		$content = $this->ajax_scroll ? "<div id='wpfb-list-wrap-$uid'>" : ""; 
		$content .= $this->ParseHeaderFooter($this->header, $uid);
		if($this->ajax_scroll) {
			$this->current_list->hide_pagenav = true;
			$is_table = (strpos($content, "<table") !== false);
		}
		...
		if($page_break && $this->ajax_scroll) {
			if(!$is_table) {
				$ajax_wrapper_id = "wpfb-ajax-scroll-$uid";
				$loader_style = empty($this->ajaxloader_cssclass) ? "style=\"background: url('".WPFB_PLUGIN_URI."images/ajax-list-loader.gif') no-repeat center bottom; width:auto; height: 30px; display:none;\"" : "style=\"display:none;\" class=\"{$this->ajaxloader_cssclass}\"";
				$content .= "<div id='$ajax_wrapper_id'></div><div id=\"ajax-loader-$uid\" $loader_style>"; // style='display:none;'
			} else {
				$loader_style = empty($this->ajaxloader_cssclass) ? "style=\"background: url('".WPFB_PLUGIN_URI."images/ajax-list-loader.gif') no-repeat center bottom; width:auto; height: 30px; display:none;\"" : "style=\"display:none;\" class=\"{$this->ajaxloader_cssclass}\"";
				$content .= "<tr id=\"ajax-loader-$uid\" $loader_style><td colspan='10'></td></tr>";			
			}
		}
...
var ajaxData$uid = ".json_encode($ajax_data).";
function wpfb_listOnScroll{$uid}(){
	if(list$uid.complete) return;
	var w = jQuery(window), t = ".($is_table ? "jQuery('#wpfb-list-wrap-$uid').children('table')" : "jQuery('#{$ajax_wrapper_id}')").";
	if( (w.scrollTop()+w.height()) >= (t.offset().top+t.height()) ){
		if(list$uid.loading) return;
		list$uid.loading = true;
		jQuery('#ajax-loader-$uid').show();
		jQuery.ajax({url:wpfbConf.ajurl,data:ajaxData$uid}).done(function ( data ) {
			jQuery('#ajax-loader-$uid').hide();
			list$uid.loading = false;
			ajaxData$uid.wpfb_list_page++;
			if(!data || data == '') {
				list$uid.complete = true;
				return;
			}
			". ($is_table ? "jQuery('#ajax-loader-$uid').before(data);" : "jQuery('#$ajax_wrapper_id').append(data);")."
			wpfb_listOnScroll{$uid}();
		});
	}
}
jQuery(window).scroll(wpfb_listOnScroll{$uid});
//]]>\n</script>\n";
		}
		return $content;
	}
