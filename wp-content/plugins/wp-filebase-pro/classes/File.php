 ...
 */
	...
	{	
		...
		foreach($sec_cats as $sc) $sc->NotifyFileRemoved($this);
    ...
    {		
		switch($name) {
			...
			case 'file_url':			return htmlspecialchars($this->GetUrl(false, (WPFB_Core::$settings->file_page_proxy && WPFB_Core::$file_details_page != $this->file_id && !is_admin())));
			case 'file_page_url':		return htmlspecialchars($this->GetUrl(false, true));
			...
			case 'file_sec_cats':		$c=''; foreach($this->GetSecondaryCats() as $sc) $c.=$sc->GetTitle().', '; return htmlspecialchars(substr($c,0,-2));
			...
			case 'dl_countdown':		return (WPFB_Core::$settings->file_page_countdown > 0) ? ('<span id="dl_countdown">'.WPFB_Core::$settings->file_page_countdown.'</span>') : 0;
		}
	...
	function Download()
	{
		...
		if($this->file_direct_linking != 1) {			
			...
			if(!WPFB_Download::RefererCheck()) {
				...
				if(empty($url) || $this->file_direct_linking == 2) $url = $this->GetUrl(false, true); // file_direct_linking=2 means redirect fo file page
				wp_redirect($url);
				exit;
			}
		}
		...
		if(!empty($this->file_password) && (empty($_POST['file_password']) || stripslashes($_POST['file_password']) != $this->file_password))
		{
			...
			$output = '<form action="" method="post"><p>' . __("This file is password protected. To download it please enter your password below:",WPFB) . '</p>
			...
			wp_die($output, __('Password Required',WPFB), array( 'response' => 403 ));
		}
	...
	function GetRemoteUri() {
			...
			if( ($rmeta = $this->GetRemoteSyncMeta()) && mysql2date('G', $rmeta->uri_expires) < time() )
			{
				wpfb_loadclass('RemoteSync');
				$rsync = WPFB_RemoteSync::GetSync($rmeta->rsync_id);
				if(!$rsync) wp_die(sprintf('Could not obtain download URI from Cloud Service. Cloud Sync (ID: %s) not found!', $rmeta->rsync_id));
				WPFB_RemoteSync::GetSync($rmeta->rsync_id)->RefreshDownloadUri($this, true);
			}
	...
	function SetRemoteSyncMeta($rev, $rsync_id, $uri_expires)
	{
		global $wpdb;
		return $wpdb->replace($wpdb->wpfilebase_rsync_meta, array(
				...
				'rev' => $rev,
				'rsync_id' => $rsync_id,
				'uri_expires' => gmdate( 'Y-m-d H:i:s', $uri_expires )
		));
	}
	...
	{
		global $wpdb;
		return $wpdb->get_row("SELECT rev,rsync_id,uri_expires FROM $wpdb->wpfilebase_rsync_meta WHERE file_id = ".(int)$this->file_id);
	}
  ...
  $post=array('menu_order'=>$this->file_attach_order,'comment_status'=>WPFB_Core::$settings->file_page_comments?'open':'closed','ping_status'=>'closed','post_author'=>$this->file_added_by,'post_content'=>WPFB_Core::$settings->file_page_gen_content?implode(" ",array_filter((array)$this,'is_string')):'','post_date'=>gmdate('Y-m-d H:i:s',min($this->GetModifiedTime(),time())),'post_name'=>$this->file_name,'post_password'=>empty($this->file_password)?null:$this->file_password,'post_status'=>$this->file_offline?'pending':'publish','post_title'=>$this->file_display_name,'post_type'=>'wpfb_filepage','tags_input'=>$this->file_tags,);if($this->file_wpattach_id>0)$post['ID']=$this->file_wpattach_id;$id=wp_insert_post($post,false);if(!$id||is_wp_error($id)){$this->file_wpattach_id=0;return false;}$this->file_wpattach_id=$id;unset($post);return $id;  	}
