...
static function Parse($tpl)
{
	...
	$tpl = preg_replace_callback('/<\?php\s+(.+?)\s*\\?>/', array(__CLASS__,'StashPhpCode'), $tpl);
	...
	$tpl = preg_replace_callback('/<!--PHP_([0-9a-f]{40})-->/', array(__CLASS__,'UnStashPhpCode'), $tpl);
...
static $php_codes;
static function StashPhpCode($ms) {
	$code =& $ms[1];
	$hash = sha1($code);
	$code = str_replace('\"','"' , $code);
	$code = str_replace("'","\\'" , $code);
	self::$php_codes[$hash] = $code;
	return "<!--PHP_$hash-->";
}

static function UnStashPhpCode($ms) {
	return "'.eval('ob_start();{\$file=&\$f; ".self::$php_codes[$ms[1]]." } return ob_get_clean();').'";
}
}
?>