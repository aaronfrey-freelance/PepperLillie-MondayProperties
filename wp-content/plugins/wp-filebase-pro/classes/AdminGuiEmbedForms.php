...
class WPFB_AdminGuiEmbedForms {
	...
	static function Display()
	{
	    ...
	    if(!WPFB_Core::$settings->frontend_upload) {
	    	echo "
	<div id='ebksel-warning' class='updated fade'><p><strong>".__('Front end upload disabled!',WPFB)."</strong> ".sprintf(__("Embedded forms will not work unless you enable front end uploads in <a href=\"%s\">Security Settings</a>",WPFB), admin_url('admin.php?page=wpfilebase_sets#'.sanitize_title(__('Security',WPFB))))."</p></div>
	";
	    }
    ?>
    <div class="wrap"> 
    <h2><?php _e('Embeddable Forms', WPFB) ?></h2>      
        <form method="post">
...
{	
	...
	?>
	<h2><?php _e(is_null($edit_form) ? 'New Embedable Form' : 'Edit Embeddable Form', WPFB); if($edit_form) echo " -  <i>".esc_html($edit_form->tag).'</i>'; ?></h2>
	...
	<tr>
		<th><label for="form-tag"><?php _e('Form Tag:', WPFB) ?></label></th>
		<td><input type="text" name="tag" value="<?php echo esc_attr(@$tag); ?>" tabindex="1" maxlength="20" /><br /><?php _e('A single word to describe the form.',WPFB); ?></td>
	</tr>
...
<?php
	printf('Make sure the roles checked here are also selected in <a href="%s">Frontend Upload Permissions</a>', admin_url('admin.php?page=wpfilebase_sets#'.sanitize_title(__('Permissions',WPFB))));
?>
	</td></tr>
	
	
	<tr>
		<th><label for="form-cat"><?php _e('Upload to category:', WPFB); ?></label></th>
		<td><select id="form-cat" name="cat_id" class="wpfb-cat-select">
			...
			<?php echo WPFB_Output::CatSelTree(array('selected' => @$cat_id, 'none_label' => __('Upload to Root',WPFB), 'add_cats' => true)); ?>
		</select></td>
	</tr>
	...
	<tr><th>Contact Form 7 Form</th>
		<td>
			...
			<?php
				array_unshift($cforms, (object)array('id' => 0, 'title' => empty($cforms) ? 'No Contact Forms found or Contact Form 7 not installed' : 'None'));
				foreach($cforms as $cform) {
					echo "<option value='{$cform->id}'"; selected($cform->id, @$cform7_id); echo ">{$cform->title}</option>";
				}
			?>
			</select><br />
	...
	<tr><th></th>
		...
		<label for="form-flash_uploader"><?php _e('Use advanced Drag &amp; Drop Uploader', WPFB) ?></label></td>
	</tr>	
	
	<tr><th></th>
		<td><input type="checkbox" id="form-extended" name="extended" value="1" <?php checked(@$extended); ?> />
		<label for="form-extended"><?php _e('Show extended input fields like Title, Version, Description etc.', WPFB) ?></label></td>
	</tr>

	<tr><th></th>
		<td><input type="checkbox" id="form-overwrite" name="overwrite" value="1" <?php checked(@$overwrite); ?> />
		<label for="form-overwrite"><?php _e('Overwrite existing files', WPFB) ?></label></td>
	</tr>
	
	<tr><th></th>
		<td><input type="checkbox" id="form-file_approval" name="file_approval" value="1" <?php checked(@$file_approval); ?> />
		<label for="form-file_approval"><?php _e('Uploaded files must been approved', WPFB) ?></label></td>
	</tr>
	
	<tr><th></th>
		<td><input type="checkbox" id="form-attach_files" name="attach_files" value="1" <?php checked(@$attach_files); ?> />
		<label for="form-attach_files"><?php _e('Attach uploaded files to post or page the form is embedded in', WPFB) ?></label></td>
	</tr>	
	
	<tr><th></th>
		<td><input type="checkbox" id="form-notify_admins" name="notify_admins" value="1" <?php checked(@$notify_admins); ?> />
		<label for="form-notify_admins"><?php _e('Notify Admins whenever a file is uploaded', WPFB) ?></label></td>
	</tr>
	...
	<tr><th><label for="form-notify_emails">More email notifications</label></th>
		...
		<br />
		<?php _e('Enter additional email addresses that will be notified on upload. Seperate by comma <code>,</code> .', WPFB) ?></td>		
	</tr>
...
class WPFB_EmbeddableForms_List_Table extends WP_List_Table {
    ...
    {
    	if($item->cat_id == -1) return '<i>'.__('Selectable by Uploader',WPFB).'</i>';
    	if($item->cat_id == 0) return '<i>'.__('Upload to Root',WPFB).'</i>';
