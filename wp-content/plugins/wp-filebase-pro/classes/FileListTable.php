...
class WPFB_FileListTable extends WP_List_Table {
    ...
    function get_columns(){
        $columns = array(
		  ...
		  $columns = array_merge($columns, WPFB_Core::GetCustomFields(true));
        return $columns;
    }
    ...
    function get_sortable_columns() {
        $sortable_columns = array(
		  ...
		  foreach(WPFB_Core::GetCustomFields(true) as $cf => $name)
			  $sortable_columns[$cf] = array($cf, false);
    ...
    function column_name($file){
		 ...
		 $actions = array(
            'edit'      => '<a href="'.$edit_url.'">'.__('Edit').'</a>',
				...
				'filepage'    => '<a href="'.esc_attr($file->GetUrl(false, true)).'">'.__('View File Page').'</a>',
        );
		 
		 if(!$file->CurUserCanDelete())
		 {
			 unset($actions['delete']);
		 }

		$cloud_sync_slug = $file->file_remote_uri ? ($rm=$file->GetRemoteSyncMeta()) ? ($sync=wpfb_call('RemoteSync','GetSync',$rm->rsync_id)) ? $sync->GetServiceSlug() : '' : '' : '';
    ...
    function column_perms($file)
    {
		return WPFB_Output::RoleNames($file->GetReadPermissions(), true)
				...
				.(strlen($file->file_password) ? ' <span class="has-pw">'.__('password',WPFB).'</span>' : '')		 ;
    }
    ...
    function process_bulk_action() {
		 ...
		 $files = isset($_REQUEST['file']) ? array_filter(array_map(array('WPFB_File','GetFile'), $_REQUEST['file']),
					create_function('$file',
							  'return ($file && $file->CurUserCan'.($this->current_action()==='delete'?'Delete':'Edit').'());')): array();
			...
			switch($this->current_action())
			{
				...
				case 'rescan':
					wpfb_loadclass('Sync');
					array_map(array('WPFB_Sync','ScanFile'),$files);
					$message = sprintf(__("%d File(s) rescanned.",WPFB), count($files));
					break;
