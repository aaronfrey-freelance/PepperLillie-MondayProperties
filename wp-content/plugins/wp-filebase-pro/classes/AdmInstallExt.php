	...
	static function PluginActionLinksFilter($action_links, $plugin) {
		...
		if(strpos($action_links[0],'button-disabled') === false) {
			if(!empty($plugin->dependencies_unmet)) {
				$action_links[0] = '<a class="buy-now button" onclick="return confirm(\'This extension depends on other extensions. Add the required extension first?\');" href="' . esc_attr ($plugin->dependencies_url) . '" target="_blank" aria-label="' . esc_attr( sprintf( __( 'Install extension %s' ), $plugin->name ) ) . '">'. __( 'Install' )  . '</a>';
