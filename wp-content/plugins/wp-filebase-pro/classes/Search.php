...
static function InitClass()
{
	if(WPFB_Core::$settings->search_integration) {
		...
		// dont list file posts if search disabled
		add_filter('posts_search', create_function('$sql','return "$sql AND (post_type <> \'wpfb_filepage\')";'));
	}
}
...
static function PostsSearch($sql)
{
	...
	$sql ="$sql AND (post_type <> 'wpfb_filepage' OR (".WPFB_File::GetReadPermsWhere()."))";
