	...
	if($action == 'add' || $action == 'update')
	{
		...
		if($type == 'list') {
			$data = array(
				...
				'ajax_scroll' => !empty($_POST['tpl-list-ajax']),
				'ajaxloader_cssclass' => $_POST['tpl-list-ajax-load-css'],
			);
...
<div id="wpfb-tabs">
	<ul class="wpfb-tab-menu">
		...
		<li><a href="#filepage"><?php _e('File Page', WPFB) ?></a></li>
	</ul>
	
	<div id="file" class="wrap">
	<p><?php _e('Templates used for single embedded files or file lists.',WPFB); ?></p>
	<?php self::TplsTable('file',array('filepage', 'filepage_excerpt')); ?>
	</div>
	...
	<div id="filepage" class="wrap">
	<p><?php _e('Templates used for Filepages.',WPFB); ?></p>
	<?php self::TplsTable('file', null, array('filepage', 'filepage_excerpt')); ?>
	</div>
...
<table class="form-table">
	...
	<tr class="">
		<th scope="row" valign="top"><label for="tpl-list-ajax"><?php _e('AJAX List', WPFB) ?></label></th>
		<td>
			<input type="checkbox" value="1" name="tpl-list-ajax" id="tpl-list-ajax" <?php checked($tpl->ajax_scroll); ?> onchange="jQuery('#tpl-list-file-ajax-load-css-wrap').toggle(this.checked);" /><br />
			List is loaded dynamically using AJAX, so pagination is not required. Elements are automatically loaded when scrolling to the end of the list (like Facebook timeline).
		</td>
	</tr>

	<tr class="form-field" id="tpl-list-file-ajax-load-css-wrap" style="<?php if(!$tpl->ajax_scroll) echo "display:none;"; ?>">
		<th scope="row" valign="top"><label for="tpl-list-ajax-load-css"><?php _e('AJAX loader CSS class', WPFB) ?></label></th>
		<td>
			<input id="tpl-list-ajax-load-css" name="tpl-list-ajax-load-css" value="<?php echo esc_attr($tpl->ajaxloader_cssclass); ?>" class="code" /><br />
			(Leave empty to use the default gif image.) CSS class for the AJAX loading animation container. This usually includes a heigth and background image declaration. You can generate AJAX loader animations at <a href="http://www.ajaxload.info/">ajaxload.info/</a>.
		</td>
	</tr>
