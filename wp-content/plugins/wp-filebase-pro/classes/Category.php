...
class WPFB_Category extends WPFB_Item {
    ...
    private function _get_tpl_var($name,&$esc)
    {
		...
		//	case 'cat_icon_url_small':	return $this->GetIconUrl('small');
			...
			case 'cat_zip_url':			return home_url('?wpfb_zip_cat='.$this->cat_id);
	...
	function SetWritePermissions($roles)
	{
		if(!is_array($roles)) $roles = explode('|',$roles);
		$roles =  array_filter(array_filter(array_map('trim',$roles),'strlen')); // remove empty
		$roles = implode('|', $roles);
		$this->cat_upload_permissions = $roles;
		if(!$this->locked) $this->DBSave();
	}
	
	function GetWritePermissions() {
		return array_filter(explode('|',$this->cat_upload_permissions));
	}
}
