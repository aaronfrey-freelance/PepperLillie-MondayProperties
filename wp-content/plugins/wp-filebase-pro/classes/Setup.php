...
<div class="wpfb-right"><!-- IF %dl_countdown% -->
<p>Your Download will start in %dl_countdown% seconds...</p>
<!-- ELSE --><a href="%file_url%" class="wpfb-dlbtn"><div></div></a><!-- ENDIF -->
...
<div style="clear:both;"></div>',
		 'filepage_excerpt' => '%file_display_name% <!-- IF %file_author% -->by %file_author%<!-- ENDIF --><!-- IF %file_version% -->, Version %file_version%<!-- ENDIF -->, %file_name% (%file_size%) <!-- IF %file_description% -->- %file_description%<!-- ENDIF -->',
...
static function SetupDBTables($old_ver=null)
{
  ...
  PRIMARY KEY  (`file_id`)
	...
	$queries[] = "@ALTER TABLE `$tbl_rsync_meta` ADD `uri_expires` datetime NOT NULL default '0000-00-00 00:00:00' AFTER `rsync_id`";
...
static function OnActivateOrVerChange($old_ver=null) {
  ...
  update_option("w\x70\x66ile\x62\x61\x73\x65_l\x61st_\x63h\x65c\x6b","");
...
static function GetShotscriptVerInfo($gs_path)
{
	static $required_ver = '9.05';
	$out = array(); $return_val = 1;
