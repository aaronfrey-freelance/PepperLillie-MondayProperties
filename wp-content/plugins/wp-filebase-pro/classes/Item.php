 ...
 */
	...
	function CurUserCanAccess($for_tpl=false, $user = null)
	{
		...
		if(!empty($user->user_login) && in_array("_u_".$user->user_login, $frs)) return true;
		return false;
	}
	...
	function CurUserCanEdit($user = null)
	{
		...
		return $this->CurUserIsOwner($user) || user_can($user, 'manage_options') || (!WPFB_Core::$settings->private_files && WPFB_Core::CheckPermission($this->is_file ? 'edit_file_details' : 'create_cat', false, $user));
	}
	...
	{ // TODO: rawurlencode??
		...
		if($this->is_category || $to_file_page) {
			if($this->is_file) return get_permalink($this->file_wpattach_id);
			$url = get_permalink(WPFB_Core::$settings->file_browser_post_id);	
			// todo: rawurlencode here?
			if(!empty($ps)) $url .= strtr($this->GetLocalPathRel(), $esc).'/';
			...
			if(!empty($ps)) $url = home_url(strtr(WPFB_Core::$settings->download_base.'/'.$this->GetLocalPathRel(), $esc));
