...
static function Schema()
{
	...
	'category_substitute' => array('default' => '', 'title' => __('<i>Category</i> Substitute',WPFB), 'desc' => __('Alternative term for the label of category drop downs in upload forms. Affects front-end only (Widgets &amp; Embedded Forms).', WPFB), 'type' => 'text'),
	...
	// download
	'file_page_proxy'		=> array('default' => false, 'title' => __('File Page Proxy', WPFB), 'type' => 'checkbox', 'desc' => sprintf(__('Redirects users to a page with file details. See <a href="%s">Filepage Template</a>', WPFB), esc_attr(admin_url('admin.php?page=wpfilebase_tpls&action=edit&type=file&tpl=filepage')))),
	'file_page_countdown'	=> array('default' => 0, 'title' => __('File Page Download Countdown', WPFB), 'type' => 'number', 'unit' => 'sec', 'desc' => __('Time the user has to wait until the download automatically starts on a file page. Use <code>%dl_countdown%</code> in Filepage Template to display the time. Set to 0 to disable this feature.', WPFB)),
	'file_page_comments'		=> array('default' => false, 'title' => __('Comments on File Page', WPFB), 'type' => 'checkbox', 'desc' => sprintf(__('If users are able to leave comments on Filepages. After changing you must run the <a href="%s">Rescan Files</a> tool.', WPFB), esc_attr(admin_url('admin.php?page=wpfilebase_manage&action=rescan')))),
	'file_page_url_slug' => array('default' => 'wpfb-file', 'title' => __('File Page URL slug', WPFB), 'type' => 'text', 'desc' => sprintf(__('The url prefix for Filepage links. Example: <code>%s</code> (Only used when Permalinks are enabled.)', WPFB), get_option('home').'/%value%/my-file-pdf/')),
	'file_page_url_wfront' => array('default' => true, 'title' => __('File Page Permalink with Front Base', WPFB), 'type' => 'checkbox', 'desc' => sprintf(__('Should the permastruct be prepended with the front base. (enabled-> /%s/, disabled->/blog/%s/)</a>', WPFB), @WPFB_Core::$settings->file_page_url_slug, @WPFB_Core::$settings->file_page_url_slug )),

	'file_page_gen_content' => array('default' => false, 'title' => __('Content Keywords', WPFB), 'type' => 'checkbox', 'desc' => sprintf(__('Put keywords in File Pages content to make them searchable by plugins.', WPFB), esc_attr(admin_url('admin.php?page=wpfilebase_manage&action=rescan')))),
   ...
   'file_browser_expanded'		=> array('default' => false, 'title' => __('Initially Expanded', WPFB), 'type' => 'checkbox', 'desc' => __('The File Browser will be fully expanded when page is loaded. Do not enabled if you have many files!', WPFB)),
		 
   'file_browser_empty_msg'		=> array('default' => __('No files available.',WPFB), 'title' => __('Empty Message', WPFB), 'type' => 'text', 'size' => 65, 'desc' => __('Message that will be displayed if file browser is empty.', WPFB)),
	...
	'enable_cat_zip'		=> array('default' => false, 'title' => __('Category ZIP Files', WPFB), 'type' => 'checkbox', 'desc' => sprintf(__('Allows users to download whole categories in a compressed zip file. Use template variable <code>%s</code>.', WPFB),'%cat_zip_url%')),
	...
	'upload_notifications' =>  array('default' => false, 'title' => __('Upload Notifications', WPFB), 'type' => 'checkbox', 'desc' => __('Notifies all blog users whenever a file they can access is uploaded.', WPFB)),
	...
	'search_results'		=> array('default' => 'pages', 'title' => __('Search Results Type', WPFB), 'desc' => __('The way search results are displayed when using the WordPress Search', WPFB), 'type' => 'select', 'options' => array('pages' => __('Show File Pages',WPFB), 'list' => __('File List',WPFB))),
	...
	'ghostscript_path' => array('default' => ''/*default is empty, will be detected!*/, 'title' => __('Ghostscript Path',WPFB), 'desc' => __('Path to Ghostscript executable used for PDF thumbnails. If ghostscript is not installed, PDF thumbnails will not be created.', WPFB), 'type' => 'text', 'class' => 'code', 'size' => 65),
	'pdf_thumbnails' => array('default' => true, 'title' => __('Generate PDF thumbnails', WPFB), 'type' => 'checkbox', 'desc' => __('Thumbnail of first page', WPFB)),
	'pdf_extract_title' =>  array('default' => false, 'title' => __('Extract PDF Title', WPFB), 'type' => 'checkbox', 'desc' => __('Using the pattern below, Titles of PDF documents will be searched and used as file_display_name', WPFB)),
	'pdf_title_regex' => array('default' => '(.+)\r?\n', 'title' => __('PDF Title Pattern',WPFB), 'desc' => __('Regular expression to find the title in the PDF Document.', WPFB), 'type' => 'text', 'class' => 'code', 'size' => 65),
	'rpc_calls' => array('default' => false, 'title' => __('Scan Files via RPC Calls', WPFB), 'type' => 'checkbox', 'desc' => __('EXPERIMENTAL. This can improve stability of sync but requires more time for each file beeing scanned.', WPFB)),
...
.NET Framework 3.5|.net35|http://www.microsoft.com/downloads/details.aspx?FamilyID=333325fd-ae52-4e35-b531-508d977d32a6",
	...
	'default_direct_linking'	=> array('default' => 1, 'title' => __('Default File Direct Linking'), 'type' => 'select', 'desc' => __('', WPFB), 'options' => array(1 => __('Allow direct linking', WPFB), 0 => __('Redirect to post', WPFB) ,2 => __('Redirect to file page',WPFB))),	 
	...
	'perm_upload_files'			=> array('default' => array('administrator','editor','author'), 'title' => __('Upload Files', WPFB), 'desc' => __('Roles and Users allowed to upload files in Admin Dashboard', WPFB), 'type' => 'roles', 'not_everyone' => true),
	'perm_edit_file_details'	=> array('default' => array('administrator','editor','author'), 'title' => __('Edit File Details', WPFB), 'desc' => __('Roles and Users allowed to edit file details of files uploaded by others', WPFB), 'type' => 'roles', 'not_everyone' => true),
	'perm_delete_files'			=> array('default' => array('administrator','editor','author'), 'title' => __('Delete Files', WPFB), 'desc' => __('Roles and Users allowed to delete files uploaded by others', WPFB), 'type' => 'roles', 'not_everyone' => true),
	
	'perm_create_cat'			=> array('default' => array('administrator','editor','author'), 'title' => __('Create Categories', WPFB), 'desc' => __('', WPFB), 'type' => 'roles', 'not_everyone' => true),
	'perm_delete_cat'			=> array('default' => array('administrator','editor','author'), 'title' => __('Delete Categories', WPFB), 'desc' => __('', WPFB), 'type' => 'roles', 'not_everyone' => true),
	
	'perm_frontend_upload'		=> array('default' => array('administrator','editor','author'), 'title' => __('Frontend Upload', WPFB), 'desc' => __('Roles and Users allowed to upload files with the Upload Widget or Embedded Forms', WPFB), 'type' => 'roles'),
	
	'perm_manage_templates'		=> array('default' => array('administrator','editor'), 'title' => __('Manage Templates', WPFB), 'desc' => __('', WPFB), 'type' => 'roles', 'not_everyone' => true),
	
	'perm_manage_rsyncs'		=> array('default' => array('administrator','editor'), 'title' => __('Manage Remote Syncs', WPFB), 'desc' => __('', WPFB), 'type' => 'roles', 'not_everyone' => true),
	'perm_manage_forms'			=> array('default' => array('administrator','editor'), 'title' => __('Manage Forms', WPFB), 'desc' => __('', WPFB), 'type' => 'roles', 'not_everyone' => true),
