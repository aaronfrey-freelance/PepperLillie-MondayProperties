...
-->
	<tr class="form-field">
		<th scope="row" valign="top"><label for="file_sec_cat1"><?php _e('Secondary Categories', WPFB) ?></label></th>
		<td colspan="3">
			<select name="file_sec_cat1" id="file_sec_cat1" class="postform wpfb-cat-select"><?php echo WPFB_Output::CatSelTree(array('selected'=>$file->file_sec_cat1, 'add_cats' => true
	, 'check_add_perm'=>true
			)) ?></select> &nbsp;
			<select name="file_sec_cat2" id="file_sec_cat2" class="postform wpfb-cat-select"><?php echo WPFB_Output::CatSelTree(array('selected'=>$file->file_sec_cat2, 'add_cats' => true
	, 'check_add_perm'=>true
			)) ?></select> &nbsp;
			<select name="file_sec_cat3" id="file_sec_cat3" class="postform wpfb-cat-select"><?php echo WPFB_Output::CatSelTree(array('selected'=>$file->file_sec_cat3, 'add_cats' => true
	, 'check_add_perm'=>true
			)) ?></select> 
		</td>
	</tr>
...
-->
	...
	<?php if(WPFB_Core::$settings->languages) { ?>
		...
		<td>
			<fieldset><legend class="hidden"><?php _e('Direct linking') ?></legend>
				...
				<br /><label title="<?php _e('No') ?>"><input type="radio" name="file_direct_linking" value="2" <?php checked('2', $file->file_direct_linking); ?>/> <?php _e('Redirect to File Page', WPFB) ?>
				<?php if($update) {
					echo ' (<a href="'.esc_attr($file->GetUrl(false, true)).'" target="_blank">';
					_e('View');
					echo '</a>)';
				} ?>
					</label>
	...
	?>
	<tr class="form-field">
		<th scope="row" valign="top"><label for="file_password"><?php _e('File Password',WPFB) ?></label></th>
		<td colspan="3"><input name="file_password" id="file_password" type="text" value="<?php echo esc_attr($file->file_password); ?>" size="40" maxlength="16" autocomplete="off" /></td>
	</tr>	
	<?php 
